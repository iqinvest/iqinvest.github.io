...
contract Math1_test {
    ...
    function add() public {
        ...
        payload = abi.encodeWithSignature("add(uint256,uint256)", b, a);
        (success,) = address(proxy).call(payload);
    ...
    function mul() public {
        ...
        payload = abi.encodeWithSignature("mul(uint256,uint256)", b, a);
        (success,) = address(proxy).call(payload);
        Assert.ok(!success, "10 * (2**256 - 10)");
    ...
    function div() public {
        ...
        Assert.equal(a.div(1), a, "(2**256 - 10) / 1");
